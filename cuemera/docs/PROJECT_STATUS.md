...
| Goal Selection (now in-camera) | Done, redesign pending confirmation | 100% (current chip-row picker); pending | `PhotographyGoal` selection currently lives in a compact horizontal chip-row picker inside `camera_screen.dart`'s header, reading/writing `selectedGoalProvider` directly. `photography_goal.dart`/`goal_providers.dart` unchanged and still the single source of truth. `goal_card.dart` no longer holds the unused `GoalCard` — it was overwritten with the public `EditorialMenuCard` used by the home screen. **This session, a redesign was prompted** (not yet confirmed applied/tested) to replace the in-camera chip row with a single tappable pill opening a modal bottom sheet list, plus a new custom top navbar (difficulty/settings, mode selector, sample/reference photo picker) and `SafeArea` fixes so the Capture button and top content don't collide with Android system UI |
...
| 10. Album Director (album feature) | Near done | ~90% | AlbumState, Shot, album_screen grid, diversity score, next-shot-type suggestion, shot detail view with per-category ScoreBadge breakdown; now also reachable directly from the home screen's "Album" card, not only via the in-camera AlbumButton |
...
- **A proposed debug FPS/condition-breakdown overlay and a runtime-tunable auto-capture threshold were discussed but never implemented** — the user explicitly paused before that prompt was run. No `kDebugPerfOverlay` flag, `debug_flags.dart`, or threshold provider exists; `_minTrackingProgress` remains a private compile-time constant in `auto_capture_service.dart`.
...
- **`camera_screen.dart` dispose-race crash** — leaving `CameraScreen` (specifically via the Android system back button/gesture, since there is no in-app back button) throws `Bad state: Cannot use "ref" after the widget was disposed` from `_onFrame`, the `CameraController.startImageStream` callback. A fix was prompted this session (`mounted` guards at the top of every `ref`-using stream callback, earlier `stopImageStream()` call in `dispose()`, and a `PopScope` to stop the stream before pop completes) but is **not yet confirmed applied or tested**.
- **`CameraScreen` top-of-screen UI has unresolved safe-area issues** — the Capture button can be covered by the Android system navigation bar, and the top content (goal chip row) can collide with the system status bar. A redesign was prompted this session (proper `SafeArea`/inset handling, the horizontal goal chip row replaced with a tappable pill + modal bottom sheet list, a new custom top navbar with difficulty/settings, mode selector, and sample/reference photo picker icons) but is **not yet confirmed applied or tested** — some of the 3 new navbar icons were expected to ship as functional stubs pending further wiring decisions on what they should actually do.
...
| fix (prompted, unconfirmed): camera_screen.dart dispose-race crash | Prompted `mounted` guards on `_onFrame` and other ref-using stream callbacks, earlier `stopImageStream()` in dispose(), and a PopScope to stop the image stream before pop completes, to resolve a "Bad state: Cannot use ref after widget disposed" crash on leaving CameraScreen via Android system back |
| redesign (prompted, unconfirmed): camera_screen.dart top UI overhaul | Prompted SafeArea/inset fixes for the Capture button and top content, replacing the goal chip row with a tappable pill + modal bottom sheet list, and adding a new custom top navbar (difficulty/settings, mode selector, sample/reference photo picker) |

# 7. Suggested Next Steps

1. **Confirm and test this session's two pending `camera_screen.dart` changes**: the dispose-race crash fix (`Bad state: Cannot use "ref" after the widget was disposed` on leaving via Android system back) and the top-UI redesign (SafeArea fixes for the Capture button/top content, chip-row → bottom-sheet goal picker, new top navbar with difficulty/settings, mode selector, sample/reference photo picker). Neither has been confirmed applied or tested as of this update — verify actual file state and behavior on-device before treating either as done.
2. **Decide what the 3 new top-navbar icon buttons should actually do.** They were expected to ship as functional stubs — difficulty/settings adjustment, mode selector, and sample/reference photo picker currently have no clear target screen/provider; needs a follow-up task to wire real behavior (or confirm a mode concept is even needed vs reusing the existing goal picker).
3. **Perform real-device testing.** Validate actual FPS, battery drain, permission flow behavior, and sanity-check the `LightAnalyzer` heuristics (negative space, symmetry, clutter, light direction) against obviously-good vs obviously-bad framing on physical hardware — now also including a direct check of the `EditorialMenuCard` performance fix (BackdropFilter sigma reduction, shadow collapse, RepaintBoundary) using `flutter run --profile` + DevTools Performance overlay to confirm the reported lag is actually resolved.
4. **Enrich `editorial_rules.dart` content.** Expand the rule set per `PhotographyGoal` once real-device data confirms which signals are reliably available and worth acting on.
5. **Wire `MemoryService` into `priority_engine.dart` for personalization.** Implement the read side so `getNextAction()` can be biased by `getFrequentCorrections()`/`getHabit()`, closing the loop left as a placeholder hook.
6. **Clean up remaining minor gaps**: remove or repurpose the now-likely-unused `goalSelectionTitle`/`goalSelectionSubtitle` strings, and either add a real `assets/images/logo.png` or intentionally commit to the text-wordmark-only splash treatment.
7. **Only after the above are validated**, expand scope: add more photography styles/rule depth, consider a Java/Spring backend, evaluate a GenAI on-device fallback, and revisit the previously-paused debug overlay / runtime-tunable threshold work if still desired.
```