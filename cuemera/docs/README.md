...
**Still not confirmed active in real use (twelfth session):** `HF_TOKEN` is present, but no call site for `CoachingPhraseModelService.ensureInstalled()` was found across the six voice-director/settings files reviewed this session, and `aiCoachingSettingsProvider.enabled`'s default is unknown (`ai_coaching_providers.dart` not reviewed this session). Either one, on its own, would fully explain phrases still not being heard. See §13.
