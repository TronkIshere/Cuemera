...
- The `pubspec.yaml` `assets:` list is missing the `espeak-ng-data/` subdirectory entries — Flutter does not bundle asset directories recursively, so every nested subdirectory needs its own explicit line, and the exact subdirectory structure (which varies per model) hadn't been supplied as of this writing.
