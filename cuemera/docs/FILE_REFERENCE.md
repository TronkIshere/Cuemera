...
`TrackingEngine` — constructed with a `DetectionThresholds thresholds` (default `DetectionThresholds.defaultValues`). `smoothSubject`/`smoothScene` unchanged. `trackingProgress()` guards both `eyesOpen` and `expression` on *both sides non-null* before adding them to the weighted average, so they cleanly drop out now that they're permanently `null` — this behavior was already correct and is locked in by test.
...
**`pose_analyzer.dart` changed this (eleventh) session:** now runs the same `PoseLandmarkGate` (`core/pose/landmark_gate.dart`) the reference-photo side uses — closing the "no landmark-likelihood gate on the live side" gap flagged in the tenth session's `README.md` §11 — plus the same hip-to-ankle `bodyRatio` extrapolation guard `reference_image_analyzer.dart` already had. `PoseAnalyzer`'s constructor is no longer `const` (it now holds a `GateHold` instance, disabled by default — any existing `const PoseAnalyzer()` call site will need updating to `PoseAnalyzer()`). `scene_profile.dart` unchanged. **Unchanged this (twelfth) session** — the pose-detector-model and crop-reconciliation changes this session were both scoped to the reference-photo path only.
...
**Two bugs found and fixed (tenth session):**
...
**Reviewed again this (twelfth) session, no code changes — opened an AI-coaching investigation.** `HF_TOKEN` is now confirmed present at build time, ruling out the most obvious reason phrases still aren't heard. This file's own `ai_gate:` logging is exactly what's needed to disambiguate the two remaining candidate causes (see `coaching_phrase_model_providers.dart`/`ai_coaching_providers.dart` entries and `README.md` §13) — next session should start from a real run of that log line.

## features/voice_director/models

#### `coaching_decision.dart`
`CoachingDecision` — `attribute`/`direction`/`tier`/`normalizedSeverity`/`fallbackPhrase`/`targetExpression`, plus `severityBand`/`dedupeKey` getters. Added `faceYaw` to `CoachingAttribute` in the eighth session — see `reference_comparison_engine.dart`'s `_evaluateFaceYaw`. **Reviewed again this (twelfth) session as part of the AI-coaching investigation, unchanged** — nothing in this file bears on why phrases aren't being heard.

## features/voice_director/services

#### `coaching_phrase_model_service.dart`
`CoachingPhraseModelService` — wraps Gemma 3 270M via `flutter_gemma`/`flutter_gemma_mediapipe`. `isReady` is `_model != null`, which only becomes true after `ensureInstalled()` completes. **Reviewed again this (twelfth) session as part of the AI-coaching investigation:** no call site for `ensureInstalled()` was found across the six voice-director/settings files reviewed this session — if none exists anywhere in the app (unconfirmed; `ai_coaching_providers.dart`, which is believed to call it, wasn't itself reviewed this session — see that entry), the model would simply never be downloaded regardless of `HF_TOKEN` being present, which would fully explain phrases not being heard on its own. See `README.md` §13.
...
**Reviewed again this (twelfth) session, unchanged.** `HF_TOKEN` confirmed present at build time this session, so this provider is confirmed returning a real `CoachingPhraseModelService` instance rather than `null` — narrows the AI-coaching investigation (see `README.md` §13) to the two remaining candidates covered in `coaching_phrase_model_service.dart`'s and `ai_coaching_providers.dart`'s entries.
