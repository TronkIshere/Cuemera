...
#### `camera_screen.dart`
The largest file in the app; hosts the live camera session. `_performCapture()` checks `cameraService.lastGallerySaveSucceeded == false` and sets `gallerySaveWarningProvider` if the gallery save failed; also calls `appTtsServiceProvider`'s `speak('Photo captured.')` right after a successful capture (before the reference-lookup/scoring logic, so it fires regardless of whether a reference photo is selected) — **updated this (seventh) session** from `ttsServiceProvider` to `appTtsServiceProvider` so this confirmation benefits from the sherpa_onnx/flutter_tts fallback too. `build()` watches `mlKitAvailabilityListenerProvider`/`mlKitUnavailableProvider` for the persistent banner, and `ref.listen`s `gallerySaveWarningProvider` for a one-shot `SnackBar` (deferred via `addPostFrameCallback` to avoid a `markNeedsBuild`-during-build crash). `TargetZoneOverlay` removed from the `Stack`.
