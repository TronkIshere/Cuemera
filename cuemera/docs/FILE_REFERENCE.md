...
`ReferenceImageAnalyzer.analyze(imagePath) → ReferenceProfile` — `bodyRatio` requires `nose`/`leftHip`/`leftAnkle` to pass the same `_minLandmarkLikelihood` confidence filter as `poseLandmarkPoints`, plus its own extrapolation-distance guard (`_maxExtremityExtrapolationMultiplier = 4.0`: an ankle farther than 4x the shoulder-width scale from the hip is treated as an unreliable extrapolation and skipped). The five independent analysis steps (pose, face, segmentation, decode, palette) run concurrently via `Future.wait`.

**Bug found and fixed this (tenth) session:** `shoulderAngleDegrees`/`torsoScale`/`shoulderBalanceRatio`/`bodyYawEstimate` were computed straight from `leftShoulder`/`rightShoulder` with **no** `likelihood` check at all — inconsistent with `bodyRatio` (above) and with `poseLandmarkPoints` (which does filter by `likelihood >= 0.6`). A shoulder landmark ML Kit itself flags low-confidence, and which the UI correctly declines to draw, could still silently drive the numeric shoulder-angle comparison used for coaching and auto-capture. Fixed: added the same `likelihood >= _minLandmarkLikelihood` gate before computing these four fields. **Still flagged, not fixed:** (1) the equivalent gate hasn't been applied to the live-camera side (`pose_analyzer.dart`) — needs a current copy of that file; (2) reference photos with implausible-but-confident landmark positions (e.g. a full-screen social-media screenshot where the subject's lower body isn't actually in frame, so ML Kit extrapolates leg landmarks into the surrounding UI chrome with high reported confidence) still pass this likelihood gate untouched — see `README.md` §11 and the standalone `cuemera-reference-photo-stability-brief-en.md` handoff document for the scoped fix directions, none yet implemented.
...
`ReferenceAnalysisPainter._findSuspectLandmarks` — **correction to this doc's previous description**, which understated the actual logic: this is not a flat 4x-length cutoff. It accumulates weighted "suspicion evidence" per landmark from four independent signals (segment length vs. a `_bodyUnit` scale derived from the median-then-largest-plausible candidate across several body measurements with outlier rejection; a symmetric left/right pair collapsing toward the same point; a large bilateral length mismatch between a mirrored limb pair; a limb crossing into the opposite body half beyond a margin, with a relief term so legitimately crossed arms don't false-positive), only hiding a landmark once evidence crosses a fixed threshold, with a safety valve that backs off entirely if more than 60% of present body landmarks would be flagged at once. Presentational-only — doesn't touch `reference_image_analyzer.dart`'s scoring pipeline; `bodyRatio` there has its own, differently-implemented extrapolation guard, and `shoulderAngleDegrees` had none until this (tenth) session (see that file's entry above). Painter logic itself unchanged this session.
