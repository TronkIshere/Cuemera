...
#### `reference_photo_crop_redetect.dart` *(new, eleventh session)*
`reconcileWithCrop` — Phase 2 of `REFERENCE_TRUST_FILTER_SOLUTION.md`. Crops to `computeSubjectBoundingBox`'s result, re-runs `PoseDetector` on just the crop (temp JPEG written to `Directory.systemTemp`, deleted after use), shifts recovered landmarks back into the original photo's coordinate space (a pure offset add — the crop only translates, doesn't resize), and merges them into the original gate's trusted set, then re-runs the full gate over the merged set. Triggered by `shouldAttemptCropRedetect` (⅓ of tracked extremities untrusted — untuned first guess). **Bug found and fixed this session, via a real test screenshot, not just by inspection:** the post-crop trust gates (`cropGate`, `mergedGate`) were built via `PoseLandmarkGate.fromRawLandmarks(...)` with no `maskSignal` argument, silently defaulting to `MaskTrustSignal.none` (`bypassed: true`). Phase 2 was therefore recovering landmarks on likelihood + geometry alone — the two signals Phase 1 exists specifically because they're insufficient — which let it recover the exact hip→knee→ankle chain Phase 1's real mask check had just correctly rejected on the test screenshot. Fixed: both gates now sample a real `MaskTrustSignal` before deciding trust, mirroring `_derivePose`'s existing likelihood-gate-then-mask-gate pattern.
...
**Changed this (eleventh) session:** `ReferenceAnalysisPainter` no longer carries its own copy of the landmark-suspicion logic. `_findSuspectLandmarks`/`_bodyUnit`/`_torsoCenterX`/`_usablePointAt` (~200 lines) moved out to `core/pose/landmark_gate.dart` as shared functions — see that entry above — unchanged in logic/thresholds, only relocated. The painter now calls `PoseLandmarkGate.fromPoints()` and draws exactly what it returns, closing the gap the note below describes (drawn skeleton and the numeric comparison fields could previously disagree, since only the painter ever consulted this logic).

Previously (through the tenth session), this doc described the heuristic as follows — kept here since the logic itself is unchanged, just relocated: not a flat 4x-length cutoff, but accumulated weighted "suspicion evidence" per landmark from four independent signals (segment length vs. a `_bodyUnit` scale derived from the median-then-largest-plausible candidate across several body measurements with outlier rejection; a symmetric left/right pair collapsing toward the same point; a large bilateral length mismatch between a mirrored limb pair; a limb crossing into the opposite body half beyond a margin, with a relief term so legitimately crossed arms don't false-positive), only hiding a landmark once evidence crosses a fixed threshold, with a safety valve that backs off entirely if more than 60% of present body landmarks would be flagged at once.
