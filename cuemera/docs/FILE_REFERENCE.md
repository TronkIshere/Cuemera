...
`PriorityAction` — **AI Integration Track 2, Phase 0, done this session.** Gained a required `decision` field (`CoachingDecision`, from the new `models/coaching_decision.dart`) alongside the existing `phrase`/`severity`/`sourceLayer`. Decouples the coaching decision from the phrase text so a future phrase-generation step (Phase 2) can vary the spoken wording without needing a new action shape.

#### `reference_comparison_engine.dart`
`ReferenceComparisonEngine.evaluate(...)` — substantially rebuilt in an earlier session: 3-tier priority fallthrough (Pose & Face → Composition → Lighting/Color), severity-tiered phrases via `_tieredPhrase`, new directional evaluators (`_evaluateFaceRoll` left/right gated behind `_faceRollDirectionIsMirrored`; `_evaluateMouthOpen`/`_evaluateEyeOpen` gained direction), `_evaluateExpression` naming the actual target label. `_evaluateBodyRatio`/`_evaluateSymmetry`/`_evaluateHue` remain single-direction by design. **AI Integration Track 2, Phase 0, done this session:** every `_evaluate*` method now also builds a `CoachingDecision` (`attribute`/`direction`/`tier`/`normalizedSeverity`/`fallbackPhrase`/`targetExpression`) alongside the phrase it already built — zero behavior change, byte-for-byte identical phrase output. `_AttributeEvaluation` no longer stores `severity`/`phrase` directly; both are now getters derived from `decision`, so they can't drift out of sync. The severity-band ceilings (`_mildSeverityCeiling`/`_moderateSeverityCeiling`) are now aliases of the single source of truth on `CoachingDecision` itself, so `decision.severityBand` always agrees with which tiered phrase text was actually picked. Direction mapping: `bodyRatio`/`symmetry`/`hue` → `CoachingDirection.none` (single-direction by design, unchanged); `faceRoll` → `left`/`right`; `expression` → `none` with `targetExpression` set; everything else → `increase`/`decrease`. **Now covered by tests** — `reference_comparison_engine_test.dart`'s new `CoachingDecision fields` group, covering `attribute`/`direction`/`tier`/`severityBand`/`targetExpression`/`dedupeKey` directly rather than only string-matching phrases; the old phrase-based tests are untouched and still pass, since phrase output is unchanged.

#### `voice_providers.dart`
`voiceDirectorListenerProvider` — **AI Integration Track 2, Phase 0, done this session.** Dedupe switched from `next.phrase == lastPhrase` (string equality) to `next.decision.dedupeKey == lastDedupeKey`. Behavior is identical today — phrase text is a deterministic function of exactly the same inputs `dedupeKey` encodes (attribute + direction + severity band, or attribute + target expression) — but this is what lets Phase 2 vary the model-generated phrase text for the same underlying decision without breaking dedupe (which keyed on the now-variable text before). `nextActionProvider`/`referenceComparisonEngineProvider` unchanged.

## features/voice_director/domain/models

#### `coaching_decision.dart` *(new)*
`CoachingDecision` — **AI Integration Track 2, Phase 0, added this session.** Immutable model: `attribute` (`CoachingAttribute` enum, one per `_evaluate*` method), `direction` (`CoachingDirection`: `increase`/`decrease`/`left`/`right`/`none`), `tier` (`CoachingTier`: `poseAndFace`/`composition`/`lighting`), `normalizedSeverity` (double), `fallbackPhrase` (today's exact hand-authored string — used verbatim now, and again in Phase 2 whenever model generation is slow/unavailable/unsupported), and nullable `targetExpression` (only set for the `expression` attribute). Exposes `severityBand` (mild/moderate/strong, via the ceiling constants that live here as the single source of truth) and `dedupeKey` (identity for de-duplicating repeated coaching prompts — see `voice_providers.dart`).

## features/voice_director/services

#### `coaching_phrase_model_service.dart` *(new)*
`CoachingPhraseModelService` — **AI Integration Track 2, Phase 1, added this session.** Wraps Gemma 3 270M via `flutter_gemma`/`flutter_gemma_mediapipe` (the `gemma3-270m-it-q8.task` mobile build, `litert-community/gemma-3-270m-it` on Hugging Face — gated, needs a token). `ensureInstalled({onProgress})` downloads on first use (flutter_gemma caches after) and registers `MediaPipeEngine` via a guarded one-time `FlutterGemma.initialize(...)` call; throws the plugin's typed `DownloadException` on failure (401/403/etc. — a distinct, user-facing action, not swallowed). `generate(CoachingDecision) → Future<String?>` builds a short-phrase prompt from the decision's fields and **never throws** — any failure or not-yet-installed state returns `null`, giving Phase 2 an unambiguous "fall back to `decision.fallbackPhrase`" signal. **Deliberately isolated from the live coaching path** — not wired into `voiceDirectorListenerProvider` yet; that wiring, with the fallback-on-failure behavior, is Phase 2.

## features/voice_director/providers

#### `coaching_phrase_model_providers.dart` *(new)*
`coachingPhraseModelServiceProvider` — **AI Integration Track 2, Phase 1, added this session.** Sources the Hugging Face token via build-time `--dart-define=HF_TOKEN=...` (simplest of the options considered; swappable for a backend-fetched token or self-hosted mirror later without touching anything else). Exposed for Phase 1 testing/iteration only — nothing here is on the live path yet.
...
**AI Integration Track 2, Phase 1, added this session — not yet run.** Drives real native Gemma 3 270M inference through `CoachingPhraseModelService`; needs a physical device with network access and a Hugging Face token with the Gemma license accepted (`flutter test integration_test/coaching_phrase_model_smoke_test.dart --dart-define=HF_TOKEN=... -d <device-id>`). Installs the model, then generates a phrase for 5 hand-picked `CoachingDecision`s spanning different attributes/directions/severity bands, printing each phrase plus install/generation latency for Phase 3's use. Not a strict content assertion — the point is human review of real output, not pinned wording; only asserts `generate()`'s contract (never throws, non-empty text once ready).