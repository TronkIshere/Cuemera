// core/constants/app_strings.dart
abstract class AppStrings {
  static const appName = 'Cuemera';
  static const appTagline = 'Your AI fashion photographer';

  static const goalSelectionTitle = 'Choose Your Shot';
  static const goalSelectionSubtitle = 'Pick a style to guide your session';

  static const permissionCameraDenied = 'Camera access is required to continue';
  static const permissionRetry = 'Grant Permission';

  static const analyzingScene = 'Analyzing scene...';
  static const holdSteady = 'Hold steady';
  static const stepBack = 'Step back a little';
  static const stepForward = 'Step forward';
  static const moveLeft = 'Move left';
  static const moveRight = 'Move right';
  static const perfectFrame = 'Perfect frame';
  static const capturing = 'Capturing';

  static const albumTitle = 'Your Album';
  static const albumEmpty = 'No shots yet';

  static const errorGeneric = 'Something went wrong';
  static const errorCameraInit = 'Failed to initialize camera';
}
